...
| `archive` | 按年/月归档文章 |
| `zen` | 切换极简阅读模式 |
| `focus` | 专注模式，隐藏导航和列表，只保留正文 |
| `night` | 夜间模式，降低亮度 |
| `theme <name>` | 切换主题，如 `theme dracula` |
| `weather <city>` | 查看/设置天气城市 |
| `save` | 保存当前页面（文章 → .md，其他 → .txt） |
| `uptime` | 显示博客运行时长 |
| `tip` | 随机显示一条终端技巧 |
| `rand` | 随机跳转一篇文章 |
| `matrix` | 切换 Matrix 数字雨背景 |
| `pomodoro <min>` | 启动番茄钟，默认 25 分钟 |
| `clear` | 刷新页面 |
...
| 命令 | 功能 |
|------|------|
| `uptime` | 显示博客从首次部署至今的运行时间 |
| `tip` | 随机显示一条 Linux 终端技巧 |
| `rand` | 随机跳转到一篇文章 |
| `save` | 文章页下载 .md，其他页面保存为 .txt |
| `theme <name>` | 切换主题：terminal / catppuccin / matrix / nord / gruvbox / dracula / tokyo-night / one-dark |
| `focus` | 专注模式，隐藏导航、列表等非内容元素 |
| `night` | 夜间模式，降低亮度保护眼睛 |
| `weather <city>` | 设置天气城市，如 `weather 北京` |
