...
这套配置使用 **chezmoi** 管理，源码托管在 [GitHub](https://github.com/LuG3Zz/dotfiles)。新机器上一行命令即可恢复全部配置：
...
chezmoi diff             # 查看即将发生的变更
chezmoi apply ~/.zshrc   # 只应用单个文件
chezmoi reapply          # 重新应用所有配置
```
...
没有 Oh My Zsh 的启动性能损耗，没有插件管理器的复杂 DSL，每个文件、每行配置都清清楚楚。所有文件通过 chezmoi 管理，新机器上 `chezmoi init --apply LuG3Zz`  一行搞定。
