...
PowerShell Profile 是每次启动 Shell 时自动执行的脚本，位置在 `$PROFILE`（通常是 `~/.config/powershell/Microsoft.PowerShell_profile.ps1`）。
...
# --- 常用函数和别名 ---
function ll { Get-ChildItem -Path @args -Format wide }
function la { Get-ChildItem -Path @args -Force }
